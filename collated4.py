...
while robot_data['start']:
    ...
    (x, pan) = pixel_to_x_pan(moveList[int(len(moveList)/2)].pt)
    if valid_x_pan(x, pan):
        spot=moveList[int(len(moveList)/2)].pt
    ...
    for _ in xrange(5):
        ...
        #Wait for fish to get close
        flag=0
        count=0
        while flag!=1 and count<75:
            camera_capture = get_image()
            im= cv2.cvtColor(camera_capture, cv2.COLOR_BGR2GRAY)
            imcopy=im
            ...
            for i in keypoints:
                if i.size>30 and i.size<40 and euclideanDist(i.pt,spot)<40:
                    flag=1
                    print "go now"
                    
            count+=1
        #Don't fish in an empty spot
        #if count==100:
        #    continue
